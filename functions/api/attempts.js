...
export async function onRequestPost(context){
  ...
  if(!correct){
    ...
    await env.DB.prepare(
      'INSERT INTO mistakes (user_id,q_key,level_id,kp,error_type,wrong_count,last_wrong_at,next_review_at,mastered,review_stage,user_answer,svg) ' +
      'VALUES (?,?,?,?,?,1,?,?,0,0,?,?) ' +
...
export async function onRequestGet(context){
  ...
  var mistakes = await env.DB.prepare(
    'SELECT q_key,level_id,kp,error_type,wrong_count,last_wrong_at,next_review_at,mastered,review_stage,user_answer,svg FROM mistakes WHERE user_id=? AND mastered=0'
  ).bind(user.id).all();
